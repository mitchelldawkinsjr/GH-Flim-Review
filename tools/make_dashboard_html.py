...
<body>
  ...
  {('<h2>Player PDF</h2><p><a href=\"' + html.escape(pdf_rel) + '\" target=\"_blank\" onclick=\"if(window.gtag){gtag(\'event\', \'pdf_open\', {event_category: \'engagement\', player: \'%s\', week: \'%s\'});}\">Open PDF</a></p><object data=\"' + html.escape(pdf_rel) + '\" type=\"application/pdf\" width=\"100%\" height=\"640px\"><p>PDF preview not available. <a href=\"' + html.escape(pdf_rel) + '\" onclick=\"if(window.gtag){gtag(\'event\', \'pdf_download\', {event_category: \'engagement\', player: \'%s\', week: \'%s\'});}\">Download</a></p></object>' % (html.escape(player), html.escape(str(week_val) if week_val else ''), html.escape(player), html.escape(str(week_val) if week_val else ''))) if pdf_rel else ''}
