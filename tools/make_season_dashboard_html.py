...
def render_player_html(player: str, totals: dict, rates: dict, code_counts: dict, title: str, breadcrumbs_html: str = "", ga_snippet: str = "", nav_html: str = "", sparkline_html: str = "", season_data: pd.DataFrame = None, weekly_scores: list = None, weekly_grades: list = None, weekly_stats_html: str = "") -> str:
    css = """
    :root {
      --bg: #f5f7fb;
      --card: #ffffff;
...
def main():
  ...
  (function(){{
    ...
    for player in players:
        ...
        # Navigation paths: out/{season}/Season/dashboards/player.html
        # Home -> ../../index.html (season index)
        # Root -> ../../../index.html (root season selector)
        root_index = Path(out_dir).parent.parent.parent / 'index.html'
        season_index = Path(out_dir).parent.parent / 'index.html'
        try:
            home_rel = os.path.relpath(season_index, out_dir)
            root_rel = os.path.relpath(root_index, out_dir)
        except Exception:
            home_rel = '../../index.html'
            root_rel = '../../../index.html'
        try:
            season_rel = os.path.relpath(Path(out_dir) / 'index.html', out_dir)
        except Exception:
            season_rel = 'index.html'
        breadcrumbs = f"<div class=\"breadcrumbs\"><a href=\"{html.escape(root_rel)}\">Home</a> &rsaquo; <a href=\"{html.escape(home_rel)}\">Season</a> &rsaquo; <a href=\"{html.escape(season_rel)}\">Dashboards</a> &rsaquo; <span>{html.escape(player)}</span></div>"

        # Helper function to extract opponent from week directory
        def get_opponent_for_week(w_int: int, root: Path) -> str:
            """Extract opponent name from CSV files in week directory"""
            wk_dir = root / f"Wk{w_int}"
            # Try results CSV first
            csvs = sorted(glob.glob(str(wk_dir / 'results_*.csv')))
            if csvs:
                stem = Path(csvs[0]).stem  # e.g., results_Wk8_Kville
                m = re.search(r'results_Wk\d+_(.+)$', stem)
                if m:
                    return m.group(1)
            # Fallback: try prepared CSV
            prep_files = sorted(glob.glob(str(wk_dir / 'Wk*_*_prepared.csv')))
            if prep_files:
        ...
        try:
            root_rel_nav = os.path.relpath(Path(out_dir).parent.parent.parent / 'index.html', out_dir)
            home_rel_nav = os.path.relpath(Path(out_dir).parent.parent / 'index.html', out_dir)
        except Exception:
            root_rel_nav = '../../../index.html'
            home_rel_nav = '../../index.html'
        nav_html = f"<div class=\"breadcrumbs\"><a href=\"{html.escape(root_rel_nav)}\">Home</a> · <a href=\"{html.escape(home_rel_nav)}\">Season</a> · <a href=\"{html.escape(season_rel)}\">Dashboards</a></div>"

        # Build combined per-week stats table (combines Per-Game Stats and Weekly Pages)
        weekly_stats_html = ""
        if 'week' in sub.columns:
            try:
                weekly_stats_rows = []
                # Convert week to numeric for proper filtering
                sub_week_numeric = pd.to_numeric(sub['week'], errors='coerce')
                week_ints = sorted([int(w) for w in sub_week_numeric.dropna().unique() if not pd.isna(w)])
                player_file_name = f"{player.strip().replace(' ', '_')}.html"
                root = Path(out_dir).parent.parent
                
                for w_int in week_ints:
                    # Filter by numeric week value
                    week_mask = sub_week_numeric == w_int
                    week_data = sub[week_mask]
                    if week_data.empty:
                        continue
                    
                    week_catches = int(pd.to_numeric(week_data['catches'] if 'catches' in week_data.columns else pd.Series([0]), errors='coerce').fillna(0).sum())
                    week_rec_yards = int(pd.to_numeric(week_data['rec_yards'] if 'rec_yards' in week_data.columns else pd.Series([0]), errors='coerce').fillna(0).sum())
                    week_rush_yards = int(pd.to_numeric(week_data['rush_yards'] if 'rush_yards' in week_data.columns else pd.Series([0]), errors='coerce').fillna(0).sum())
                    week_drops = int(pd.to_numeric(week_data['drops'] if 'drops' in week_data.columns else pd.Series([0]), errors='coerce').fillna(0).sum())
                    week_touchdowns = int(pd.to_numeric(week_data['touchdowns'] if 'touchdowns' in week_data.columns else pd.Series([0]), errors='coerce').fillna(0).sum())
                    # Get grade (average if multiple rows)
                    week_grade = ''
                    if 'grade' in week_data.columns:
                        grades = week_data['grade'].dropna().astype(str).tolist()
                        if grades:
                            week_grade = grades[0]  # Take first grade if multiple rows
                    elif 'score' in week_data.columns:
                        # Calculate grade from score if grade column doesn't exist
                        avg_score = float(pd.to_numeric(week_data['score'], errors='coerce').fillna(0).mean())
                        week_grade = letter(avg_score)
                    
                    # Get opponent name
                    opponent = get_opponent_for_week(w_int, root)
                    week_label = f"Wk{w_int}" + (f" - {opponent}" if opponent else "")
                    
                    # Build link to weekly dashboard
                    try:
                        wk_dir = root / f"Wk{w_int}"
                    ...
                    weekly_stats_rows.append(
                        f"<tr>"
                        f"<td><a href=\"{html.escape(dash_rel)}\">{html.escape(week_label)}</a></td>"
                        f"<td>{week_catches}</td>"
                        f"<td>{week_rec_yards}</td>"
                        f"<td>{week_rush_yards}</td>"
                        f"<td>{week_drops}</td>"
                        f"<td>{week_touchdowns}</td>"
                        f"<td>{html.escape(week_grade) if week_grade else '-'}</td>"
                        f"<td>{pdf_cell}</td>"
                        f"</tr>"
                    )
                
                if weekly_stats_rows:
                    weekly_stats_html = (
                        "<h2>Per-Game Stats</h2>"
                        "<div class=\"table-wrap\">"
                        "<table>"
                        "<tr><th>Week</th><th>Catches</th><th>Rec Yards</th><th>Rush Yards</th><th>Drops</th><th>Touchdowns</th><th>Grade</th><th>PDF</th></tr>"
                        + ''.join(weekly_stats_rows) +
                        "</table>"
                        "</div>"
                    )
            except Exception as e:
                # If there's an error building the stats table, just skip it
                weekly_stats_html = ""

        html_str = render_player_html(player, totals, rates, codes, args.title, breadcrumbs, ga_snippet, nav_html, sparkline_html, sub, weekly_scores, weekly_grades, weekly_stats_html)
    ...
    try:
        root_rel_idx = os.path.relpath(Path(out_dir).parent.parent.parent / 'index.html', out_dir)
        home_rel_idx = os.path.relpath(Path(out_dir).parent.parent / 'index.html', out_dir)
    except Exception:
        root_rel_idx = '../../../index.html'
...
<body>
  <div class=\"container\">
    ...
    <div class=\"breadcrumbs\"><a href=\"{html.escape(root_rel_idx)}\">Home</a> &rsaquo; <a href=\"{html.escape(home_rel_idx)}\">Season</a> &rsaquo; <span>Dashboards</span></div>
